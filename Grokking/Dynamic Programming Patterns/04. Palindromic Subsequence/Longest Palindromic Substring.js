let findLPSLength = function (st) {
  ...
  function findLPSLengthRecursive(st, startIndex, endIndex) {
    ...
    if (st[startIndex] === st[endIndex]) {
      const between = findLPSLengthRecursive(st, startIndex + 1, endIndex - 1);
      if (between === endIndex - startIndex - 1) {
        memo[startIndex][endIndex] = 2 + between;
...
const findLPSLengthBottomUp = (st) => {
  ...
  for (let startIndex = st.length - 1; startIndex >= 0; startIndex--) {
    for (let endIndex = startIndex + 1; endIndex < st.length; endIndex++) {
      // case 1: elements at the beginning and the end are the same
      if (
        st.charAt(startIndex) == st.charAt(endIndex) &&
        dp[startIndex + 1][endIndex - 1] === endIndex - startIndex - 1
      ) {
...
console.log('Length of LPS ---> ' + findLPSLengthBottomUp('abdbca'));
console.log('Length of LPS ---> ' + findLPSLengthBottomUp('cddpd'));
console.log('Length of LPS ---> ' + findLPSLengthBottomUp('pqr'));
